...
export function CutoffSection({
  ...
  // Responsive rendering to avoid duplicate field registration
  ...
  useEffect(() => {
    ...
    const update = () => {
      setRenderDesktop(mql.matches)
      setRenderMobile(!mql.matches)
    }
    update()
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
        <div className="flex flex-col gap-1">
          <div className="flex items-center justify-between gap-2">
            ...
            {otherSalaries.length > 0 && (
              <div className="relative">
                ...
                {salaryDropdownOpen && (
                  <div className="absolute right-0 top-full mt-1 bg-card border border-line rounded-md shadow-md z-10 py-1 min-w-32">
                    {otherSalaries.map((s) => (
                      <button
                        key={s.label}
                        type="button"
          ...
          <input
            ...
            className={[
              'w-full px-3 py-3 rounded-md text-sm text-header bg-surface outline-none transition-all border shadow-sm focus:border-accent',
                      ...
                      <div className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-1.5">
                          <input
                            ...
                            className={[
                              'flex-1 px-2 py-1.5 rounded-md text-sm text-header bg-surface outline-none transition-all border shadow-sm focus:border-accent',
                    ...
                    <td className="py-2 pr-3">
                      <div className="flex flex-col gap-0.5">
                        <input
                          ...
                          onFocus={(e) => e.target.select()}
                          className={[
                            'w-full px-2 py-1.5 rounded-md text-sm text-header bg-surface outline-none transition-all border shadow-sm focus:border-accent',
                    ...
                    {customColumns.map((col) => (
                      <td key={col} className="py-2 pr-3">
                        <input
                          ...
                          className="w-full px-2 py-1.5 rounded-md text-sm text-header bg-surface outline-none transition-all border border-line shadow-sm focus:border-accent"
                        />
                      </td>
                    ))}
          ...
          <>
            {customColumns.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {customColumns.map((col) => (
                  <span key={col} className="inline-flex items-center gap-1 px-2 py-1 rounded-sm text-xs font-medium bg-surface text-muted border border-line shadow-sm">
                    {col}
                    <button
                      type="button"
                      onClick={() => removeColumn(col)}
            ...
            {itemFields.map((field, itemIndex) => {
              ...
              const isAI = aiFilledIds.has(field.id)
              return (
                <div key={field.id} className="bg-surface rounded-md p-3 flex flex-col gap-2.5 shadow-sm border border-line">
                    ...
                    <div className="flex flex-col gap-0.5 flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <input
                          ...
                          {...register(`cutoffs.${cutoffIndex}.items.${itemIndex}.name`)}
                          className={[
                            'flex-1 px-2 py-1.5 rounded-md text-sm text-header bg-card outline-none transition-all border shadow-sm focus:border-accent',
                            itemError?.name ? 'border-due-danger' : 'border-line',
                          ].join(' ')}
                        />
                        {isAI && (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <input
                        ...
                        onFocus={(e) => e.target.select()}
                        className={[
                          'w-full px-2 py-1.5 rounded-md text-sm text-header bg-card outline-none transition-all border shadow-sm focus:border-accent',
                          itemError?.amount ? 'border-due-danger' : 'border-line',
                        ].join(' ')}
                      />
                      {itemError?.amount && (
                      ...
                      {customColumns.map((col) => (
                        <div key={col} className="flex flex-col gap-0.5">
                          ...
                          <input
                            ...
                            placeholder={col}
                            {...register(`cutoffs.${cutoffIndex}.items.${itemIndex}.custom_fields.${col}`)}
                            className="w-full px-2 py-1.5 rounded-md text-sm text-header bg-card outline-none transition-all border border-line shadow-sm focus:border-accent"
                          />
                        </div>
                      ))}
                    </div>
